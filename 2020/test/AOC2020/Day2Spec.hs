...
spec = do
  describe "<AOC2020.Day2>" $ do
    ...
    it "should parse password entry" $ do
      parsePasswordEntry "1-3 a: abcde"
        `shouldBe` ((PasswordPolicy 1 3 'a'), (Password "abcde"))
    it "should return password as valid" $ do
      isValidPassword1 ((PasswordPolicy 1 3 'a'), (Password "abcde"))
        `shouldBe` True
    it "should return password as invalid" $ do
      isValidPassword1 ((PasswordPolicy 1 3 'b'), (Password "cdefg"))
        `shouldBe` False
    it "should correctly count valid passwords" $ do
      countValidPasswords isValidPassword1 testInput `shouldBe` expected1
    it "should return password as valid" $ do
      isValidPassword2 ((PasswordPolicy 1 3 'a'), (Password "abcde"))
        `shouldBe` True
    it "should return password as invalid" $ do
      isValidPassword2 ((PasswordPolicy 1 3 'b'), (Password "abade"))
        `shouldBe` False
    it "should correctly count valid passwords" $ do
